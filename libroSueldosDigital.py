...
class LibroSueldos:
    ...
    def menuConsultas(self, empresa):
        ...
        for n in range(1, 10):
            try:
                self.driver.find_element(By.ID, "ddlCUIT").click()
                representar = self.driver.find_element(By.XPATH, "/html/body/form/div[3]/div[3]/table/tbody/tr[2]/td/select/option["+str(n)+"]")
                r = representar.text
                print(r)
                print(empresa)
                # if r.find(empresa) >= 0:
        ...
        sleep(1)
        self.driver.find_element(By.XPATH, "/html/body/form/div[3]/div[5]/div[2]/div[3]/a").click()

    def periodo(self, periodo):
        self.driver.find_element(By.ID, "ctl00_ContentPlaceHolder1_InputPeriodo1_txtPeriodo").send_keys(periodo)
        self.driver.find_element(By.XPATH, "/html/body/form/div[3]/div[6]/div[4]/div[2]/input").click()

    def descargarArchivos(self):
            for c in range(2, 20):
                try:
                    liquidacion = self.driver.find_element(By.XPATH, "/html/body/form/div[3]/div[6]/div[4]/table/tbody/tr[2]/td/table/tbody/tr[" + str(c) + "]/td[5]/input")                                    
                    liquidacion.click()
                except Exception:
                    break
